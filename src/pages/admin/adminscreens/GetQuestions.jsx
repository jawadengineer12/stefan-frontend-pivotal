...
const GetQuestions = () => {
  ...
  const fetchCompanyQuestions = async (companyId) => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/admin/get-questions-by-company/${companyId}`
      );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (selectedCompanyId) {
        const res = await axios.get(
          `${API_BASE_URL}/admin/get-questions-by-company/${selectedCompanyId}`
        );
        allSections = res.data;
        const company = companies.find((c) => c.id === selectedCompanyId);
        allSections.forEach(
          (s) => (s.company_name = company?.name || "Unknown")
        );
      } else {
        const companyFetches = await Promise.all(
          companies.map(async (company) => {
            const res = await axios.get(
              `${API_BASE_URL}/admin/get-questions-by-company/${company.id}`
            );
      ...
      // CSV Header
      let exportText =
        "Company Name,Section Label,Section Title,Question,Average Rating,Standard Deviation,User Name,User Email,Rating,Answer,Submitted On\n";
      ...
      allSections.forEach((section) => {
        section.questions.forEach((q) => {
          q.answers.forEach((a) => {
            ...
            const answerText = isBlankAnswer
              ? "blank"
              : a.answer.replace(/"/g, '""');
            const rating = isBlankAnswer || a.rating == null ? "N/A" : a.rating;

            exportText += `"${section.company_name}","${section.label}","${
              section.title
            }","${q.question}","${q.avg_rating ?? "N/A"}","${
              q.std_dev ?? "N/A"
            }","${a.user_name ?? "N/A"}","${
              a.user_email ?? "N/A"
            }","${rating}","${answerText}","${a.submitted_at ?? "N/A"}"\n`;
          });
        });
      });
      ...
      {loading ? (
        ...
        [...sections]
          ...
          .map((section, sIndex) => (
            <div
              key={sIndex}
              className="mb-10 border border-gray-300 rounded-lg shadow-sm"
            >
              <div
              ...
              >
                ...
                <div>
                  <button className="text-[#548B51] font-bold text-xl cursor-pointer">
                    {expandedAll ? "−" : "+"}
                  </button>
                </div>
              </div>
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button className="text-xs font-medium bg-green-100 text-[#548B51] px-2 py-1 rounded cursor-pointer">
                              SD: {q.std_dev ?? "N/A"}
                            </button>
                            <button
